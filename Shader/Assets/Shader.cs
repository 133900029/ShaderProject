...
//变换组合
//物体坐标 世界坐标 摄像机坐标
//由于物体顶点太多。先进行世界和摄像机的合并

//线性变换：F(a+b)=F(a)+F(b)   F(ka)=kF(a)
//仿射变换：v=v1M+b
//可逆变换：F1(F(a))=a
//等角变换：变换前后，向量夹角大小和方向不变
//正交变换：轴保持垂直不缩放
//刚体变换：只改物体的位置和方向
...
//矩阵行列式
//行列式等于该行元素和对应的代数余子式的乘积和
//几何解释：等于以基变量为两边的平行四边形的面积

//矩阵的逆
//不是每个矩阵都有逆矩阵
//M*M-1=I
//M-1=代数余子式的转置  除以M的行列式
//几何解释：让转换可以撤销

//正交矩阵
//M*MT=I
//M和M的转置为I即为正交
//几何解释：当M为正交矩阵。求逆矩阵的时候直接转置可以减少计算量
//条件：每一行都是单位向量，所有行都相互垂直
...
//4d向量的w分量可以用来控制  矩阵平移部分的开关

//由于在3*3实现的变换是没有考虑到平移
//所以一般先将变换中心点平移到  原点， 变换之后再平移回去
...
//            简洁
//            任意三个数都是合法
//欧拉角缺点：表达不唯一（+360）（heading和bank要在180到-180，pitch要在90到-90）
//            求插值难 
//x或者z先转90度就容易万向锁

//unity 的y轴是  绕世界左边，x和z是绕 物体坐标，   先y 再x和z
...
//四元数
//q = [cos(o/2) (   sin(o/2)nx  sin(o/2)ny  sin(o/2)nz  )]
//负四元数
//-q = [-w -v]
//单位四元数
//q = [1, 0向量]
